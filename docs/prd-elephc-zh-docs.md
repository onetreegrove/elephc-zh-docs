...
- 不对 elephc 源码进行功能性修改。
- 不部署到 GitHub Pages；当前仅本地使用，未来部署到独立服务器。
...
- 站点不部署到 GitHub Pages；当前仅本地使用，未来部署到独立服务器。
...
- 声明中文翻译和整理内容参考上游项目许可证授权，并保留上游来源与许可证链接。
...
- Markdown 友好，适合文档站。
- 侧边栏和导航生成成本低。
- 可生成静态站点，当前用于本地预览和构建验证，未来适配独立服务器发布。
- 可先快速上线，再逐步增强视觉设计。
...
- 翻译调用方式使用 agy cli agents 流程，不在 MVP 内直接接入翻译 API 密钥。
- 允许自动化创建 PR，并由人工 review 后合并。
- `elephc-src` 需要作为 git submodule。
- 需要声明与上游项目的授权和版权关系。
- 站点名称使用“elephc 中文文档”。
- 当前仅本地使用，未来部署到独立服务器。
- 中文翻译和整理内容的许可证参考上游项目许可证。

## 15. 待确认问题

- 独立服务器的具体地址、发布账号、目录结构和发布命令。
- agy cli agents 翻译流程的批处理并发度、审校策略和失败重试策略。
